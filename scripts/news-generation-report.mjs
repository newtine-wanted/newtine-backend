...
export async function writeGenerationReport(run, path) {
  ...
  const lines = [
    ...
    `- 실행: ${run.id}`,
    `- 기준 시각: ${s.at}`,
    '- 검증·공개 전 초안입니다. 관점 개수 부족은 보류 사유로 사용하지 않습니다.',
    '',
    '## 토큰과 추정 비용',
    '',
    '| 호출 | 입력 토큰 | 출력 토큰 | 추정 비용 (USD) |',
    '| ---: | ---: | ---: | ---: |',
    `| ${s.usage.length} | ${s.usage.reduce((n, u) => n + (u.inputTokens ?? 0), 0)} | ${s.usage.reduce((n, u) => n + (u.outputTokens ?? 0), 0)} | $${cost.usd.toFixed(4)} |`,
    '',
    '- 모델: gpt-5.4-mini-2026-03-17. 2026-09-20 표준 단가: 100만 토큰당 입력 $0.75, 캐시 입력 $0.075, 출력 $4.50.',
